...
def inspect_spikes():
    ...
    try:
        ...
        if meta_row:
            meta_id = meta_row[0]
            print(f"\nChecking {sensor_name} (id={meta_id}) for Today's rows...")
            
            # Fetch all rows for today
            today_ts = datetime.datetime.now().replace(hour=0, minute=0, second=0, microsecond=0).timestamp()
            cursor.execute("SELECT start_ts, state, sum FROM statistics_short_term WHERE metadata_id = ? AND start_ts >= ? ORDER BY start_ts ASC", (meta_id, today_ts))
