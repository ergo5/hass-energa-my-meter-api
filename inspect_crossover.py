...
def inspect_crossover():
    ...
    try:
        ...
        if meta_row:
            meta_id = meta_row[0]
            print(f"\nChecking Crossover for {sensor_name} (id={meta_id}):")
            
            # Fetch LAST 5 rows of statistics (Long Term)
            cursor.execute("SELECT start_ts, state, sum FROM statistics WHERE metadata_id = ? ORDER BY start_ts DESC LIMIT 5", (meta_id,))
